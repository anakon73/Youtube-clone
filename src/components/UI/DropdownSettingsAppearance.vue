...
<template>
  <DropdownSettingsHeader @back="$emit('select-menu', 'main')" title="Appearance" />
  <section class="border-b py-2">
    <div class="text-gray-500 text-xs p-3">
      Setting applies to this browser only
    </div>
    <ul>
      <DropdownSettingsListItem v-for="(themeName, themeId) in themes" icon="check" :key="themeId" :label="themeName"
        :active="themeId === selectedOptions.theme.id" @click="selectedOption({ id: themeId, text: themeName })" />
    </ul>
  </section>
</template>
